...
**arb Profile：** `validation.prd.require_hitl: true`、`validation.design.require_hitl: true`（须 P1 实现 `prd_hitl` / `design_hitl` interrupt 后方可端到端跑通）。
...
- [ ] `python -m multi_agent_code_factory run --profile arb ...` 全链路跑通（**P1**：依赖 `prd_hitl` / `design_hitl` interrupt；Profile 路径 `domains/arb/profile/arb.yaml`）
- [ ] `prd_validate` / `design_validate` 失败可回 PM/Architect
- [ ] P1：`prd_hitl` / `design_hitl` / `deploy_hitl` 可暂停与恢复
