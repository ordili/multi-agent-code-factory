...
> **V2 — 不在 V1 实现范围。** 见 [README.md](../README.md)。  
...
> **说明：** 下列 `arb/` 为 `code_root`（`../arb-robot/`）内的 Python 包布局，与仓库根 `domains/arb/`（领域配置）无关。
...
| 改 `risk.py` / `executor.py` | 须 [deploy HITL](./arb-factory-design.md#42-套利-profiledomainsarbprofilearbyaml) 后部署 |
...
LangSmith / LLM 密钥 **不属于** arb-core（见主线 [multi-agent-pipeline-design.md §9](../../../docs/design/pipeline/multi-agent-pipeline-design.md#9-环境变量)）。
...
- [ ] 契约测试 + mock 报价测试通过
- [ ] 工厂流水线曾成功修改上述代码并通过 pytest（见 [arb-factory-design.md §5](./arb-factory-design.md#5-验收)）
