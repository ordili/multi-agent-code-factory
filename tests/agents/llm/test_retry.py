...
from multi_agent_code_factory.agents.llm.retry.policy import (
    ...
    is_retryable_prompted_json_error,
...
def test_default_retry_policy_differs_by_output_mode() -> None:
    ...
    assert default_retry_policy("prompted_json").is_retryable(LlmParseError("bad json"))


def test_is_retryable_prompted_json_error() -> None:
    assert is_retryable_prompted_json_error(LlmParseError("bad json")) is True
    assert is_retryable_prompted_json_error(ValueError("bad")) is False
