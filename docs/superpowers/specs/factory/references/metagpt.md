...
> **主线：** [multi-agent-pipeline-design.md](../../multi-agent-pipeline-design.md)
...
| 结构化产物 | JSON Schema + MD + Mermaid | 主线 §2、[artifacts/](../artifacts/README.md) |
| PRD → 设计 → 代码 | `spec` → `design` → Profile.`code_root` | [profiles.md](../profiles.md) |
| Architect 交付物 | 文件列表、接口、序列图、`dev_tasks` | [schemas/design.md](../artifacts/schemas/design.md) |
| Executable feedback | `run_tests` Tool + Parser + RetryBundle | 主线 §4.5.1、[test-report.md](../artifacts/schemas/test-report.md) |
| **Watch / 订阅** | 消息池 + `watch` | `context.py`（主线 §4.5） |
| **PM 产物** | KPI、Features、User Stories、Requirement Pool | [schemas/spec.md](../artifacts/schemas/spec.md) |
| **设计文档模板** | Google Design Doc + 七项工程 | [format-to-human/design.md](../artifacts/format-to-human/design.md) |
...
| 产物落盘 | Profile.`code_root`（仓库外）+ `docs/factory/runs/` | 主线 §6.1 |
...
| `DevManifest.reflection`（Reflexion） | [dev-manifest.md](../artifacts/schemas/dev-manifest.md) | P0 |
...
| QA `tests_missing` 检测；`auto_generate_tests` | [test-report.md](../artifacts/schemas/test-report.md) | P1 |
