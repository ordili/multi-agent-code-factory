...
> **机器真源：** [`../schemas/spec.md`](../schemas/spec.md)（`SpecArtifact` / `spec.json`）  

> **Run 路径：** `docs/factory/runs/<task_id>/spec.md`  
...
**`consistency_model` / `delivery` / `conflict_strategy` 枚举** 见 [`schemas/spec.md`](../schemas/spec.md)（Architect 实现时对照；PM 在 spec.md 用 **中文说明** 即可）。
