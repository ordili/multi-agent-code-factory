...
> **Run 路径：** `docs/factory/runs/<task_id>/spec.json`  

> **人读格式：** [format-to-human/spec.md](../format-to-human/spec.md) → `spec.md`  
...
> **PM vs Design：** `spec.json` 阶段 PM 主要填 **枚举 + `notes` 文字**；`latency` / `throughput` / `availability` / `staleness_bound` / `recovery` **留空**，由 Architect 在 `design.json` 量化。见 [format-to-human/spec.md §8](../format-to-human/spec.md#8-稳定性性能与数据一致性)。
