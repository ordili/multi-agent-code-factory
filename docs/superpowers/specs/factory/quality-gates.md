...
> **实现：** `multi_agent_code_factory/validators/` · [`validation-report.md`](./artifacts/schemas/validation-report.md)
...
**arb Profile 建议：** `validation.spec.require_hitl: true`、`validation.design.require_hitl: true`。
...
模板：[format-to-human/spec.md](./artifacts/format-to-human/spec.md)。
...
模板：[format-to-human/design.md](./artifacts/format-to-human/design.md)、[format-to-human/flow.md](./artifacts/format-to-human/flow.md)。
...
| `DES-211` | warn | Profile 为长运行服务/套利时 §10 Monitoring 非 N/A |
...
LangGraph **`interrupt_before`**；[`HitlDecision.stage`](./artifacts/schemas/hitl.md)：
