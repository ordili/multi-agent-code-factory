...
> **依据：** [00-master-overview.md](./00-master-overview.md)（仓库 **首个应用目标** 为套利机器人；**本流水线与具体业务领域无关**）  
> **代码目录：** `multi_agent_code_factory/`  
> **状态：** 草案，待评审  
> **交接原则：** 参考 MetaGPT 的 SOP 思路——角色之间用 **结构化产物** 交接，减少自由文本串联导致的幻觉。  
> **细目文档：** [factory/README.md](./factory/README.md)（Profile、Artifact、示例、参考分文件，**本文保持精简**）  
> **外部参考：** [factory/references/](./factory/references/README.md)（MetaGPT、开源调研、术语）
...
| [factory/profiles.md](./factory/profiles.md) | Profile 字段、Toolchain、Parser、多语言矩阵 |
| [factory/artifacts/](./factory/artifacts/README.md) | JSON Schema、人读格式、校验规则索引 |
| [`multi_agent_code_factory/profiles/*.yaml`](../../multi_agent_code_factory/profiles/) | Profile **配置真源**（如 `java-maven.yaml`） |
| [`multi_agent_code_factory/schemas/`](../../multi_agent_code_factory/schemas/) | Pydantic 实现 |
| [factory/quality-gates.md](./factory/quality-gates.md) | PM / Architect **规则校验** + 可选 **人工 HITL** |
| [factory/examples/](./factory/examples/README.md) | JSON / Mermaid **示例片段** |
| [factory/references/](./factory/references/README.md) | MetaGPT、开源调研、术语 |
...
| **命名** | 图节点：`{产物}_validate` / `{产物}_hitl`；配置：`validation.*`；详见 [quality-gates.md §0](./factory/quality-gates.md#0-命名约定) |
...
| **测试** | **语言无关**：Profile 配置命令 + Parser → `TestReport`（[factory/profiles.md](./factory/profiles.md)） |
| **PM/Architect 质量** | **规则校验**（`spec_validate` / `design_validate`）+ 可选 **人工 HITL**（§4.1.2、[quality-gates.md](./factory/quality-gates.md)） |
...
| 虚线 → `escalation_hitl` | `loop_limits` 触顶且 `on_limit_exceeded=escalation_hitl`（P1；**MVP 默认 `fail`** 不经此节点） |
| `deploy_hitl` | Reviewer **`approved=true`** 通过后敏感部署审批；**≠** loop 触顶 escalation |
| `RESUME` | `escalation_hitl` 批准后从 §4.3 **对应再入点**续跑（非固定回 PM） |

> 图源：[factory/examples/snippets/pipeline-overview.mmd](./factory/examples/snippets/pipeline-overview.mmd)。回路计数与 `loop_limits` 见 [§4.1](#41-流程)、[§4.4](#44-回路上限与运行预算loop_limits--budget)。
...
| 配置项 | 说明 |
|--------|------|
| `code_root` | **生成代码根目录**（Profile 指定；**须在 `multi-agent-code-factory` 仓库外**，见 [profiles.md §1.1](./factory/profiles.md#11-code_root生成代码目录)） |
...
| `toolchain` | 构建与测试命令集（[profiles.md §2](./factory/profiles.md#2-toolchain语言无关-test-层)）；含 `test_command`、`test_parser` 等 |
...
| `sandbox` | 可选；`local` \| `docker`；QA / Developer 命令执行环境（[open-source-survey.md](./factory/references/open-source-survey.md)） |
| `mcp_servers` | 可选；MCP Tool 列表（[open-source-survey.md](./factory/references/open-source-survey.md)，P2） |

完整 Toolchain、Parser 矩阵、各语言 Profile YAML 与 CLI 示例见 **[factory/profiles.md](./factory/profiles.md)**。配置真源：[`multi_agent_code_factory/profiles/*.yaml`](../../multi_agent_code_factory/profiles/)。
...
套利等业务通过 **不同 Profile + `code_root`** 注入；工厂图、Schema、路由不变。首个应用见 [00-master-overview.md](./00-master-overview.md)（如 `profiles/arb/`）。

规则校验见 [quality-gates.md](./factory/quality-gates.md)。

### 1.2 MVP 范围（v1 实现边界）

| 纳入 MVP | 不纳入 MVP（见 [metagpt.md §B.3](./factory/references/metagpt.md#b3-实现-backlog设计已定待编码) / [open-source-survey.md §C.5](./factory/references/open-source-survey.md#c5-实现-backlog)） |
|----------|--------------------------------|
| **五 Agent** + validate/HITL/Deploy **图节点**（含 `deploy_hitl`；`spec_hitl` / `design_hitl` 图节点存在，Profile 默认 `require_hitl: false`） | `spec_hitl` / `design_hitl` **强制 interrupt**（生产 Profile，P1 完善 UI） |
| **spec_validate / design_validate** MVP 规则集（[quality-gates.md](./factory/quality-gates.md)） | 全量 DES/SPEC 规则 + Mermaid 格式校验（P1；MVP 仅 Pydantic + 核心 error 规则） |
| Profile `default` + `code_root` | Docker `sandbox`、MCP |
...
| **`run_tests` Tool** → `TestReport`（默认 `junit_xml` + `exit_code_only` Parser） | `go_json` / `cargo_json` / `forge_json` Parser（P1） |
| Profile `default`（Python pytest + JUnit XML） | `solidity-hardhat` Profile（P2） |
| Profile 矩阵：`java-maven`、`java-gradle`、`rust-cli`、`solidity-foundry`（[profiles.md §3](./factory/profiles.md#3-profile-矩阵)） | 完整 `profiles/arb`（另写应用 spec） |
| LangGraph 主干 + **`route_after_test`** + **`route_after_review`** + validate 路由 | checkpoint / `resume`（P1） |
| Schema 含 `reflection`（实现可先做最小版） | `git_diff` Tool、AFlow 工作流自优化（P1/P3） |
...
- `TestReport` 来自 **`run_tests` Tool**（执行 Profile.`toolchain.test_command` + Parser），天然是结构化数据；与目标语言无关（[profiles.md §2](./factory/profiles.md#2-toolchain语言无关-test-层)）。
- MetaGPT 论文强调 **structured outputs** 降低幻觉；对外文件多为 MD，**对内/对下游** 靠结构约束（见下表）。

**Markdown 仍要写：** 与 MetaGPT 一样，给人看的 PRD、设计说明不可省；**JSON 与 MD 同源生成**（先 Structured Output，再渲染 `spec.md`），或 MD 为 JSON 的摘要。章节模板见 [factory/artifacts/format-to-human/](./factory/artifacts/format-to-human/README.md)。
...
我们借鉴：**MD + Mermaid + 代码** 给人和仓库；**JSON Schema** 给流水线程序，比 MetaGPT 更明确「机器真源」。
...
**全线图（产物 + 回路）：** 见 [§1](#1-设计结论)；`.mmd` 副本见 [pipeline-overview.mmd](./factory/examples/snippets/pipeline-overview.mmd)。`escalation_hitl` 批准后从 **§4.3 再入规则** 对应再入点续跑（非固定回 PM）。
...
流程级差异与借鉴清单见 **[metagpt.md](./factory/references/metagpt.md)**（本文不再重复表格）。

### 4.1.2 产物校验与 HITL（PM / Architect）

PM、Architect 产出在 **进入下游 Agent 之前** 须过 **spec_validate / design_validate**（程序规则）；Profile.`validation` 可配置 **spec_hitl / design_hitl**（人工，非 LLM Reviewer）。命名见 [quality-gates.md §0](./factory/quality-gates.md#0-命名约定)。

| 节点 | 类型 | 失败 / 审批 | 细目 |
|------|------|-------------|------|
| **spec_validate** | 程序 | 回 **PM** | [quality-gates.md §3](./factory/quality-gates.md#3-spec_validate--规则清单) |
| **spec_hitl** | 人工 interrupt | 驳回 → PM | `validation.spec.require_hitl` |
| **design_validate** | 程序 | 回 **Architect** | [quality-gates.md §4](./factory/quality-gates.md#4-design_validate--规则清单) |
| **design_hitl** | 人工 interrupt | 驳回 → Architect | `validation.design.require_hitl` 或 `hitl_flags` |

产物：`spec_validation.json`、`design_validation.json`（[`ValidationReport`](./factory/artifacts/schemas/validation-report.md)）。  
**与 Reviewer 分工：** validate 验 **文档**（确定性）；Reviewer（`reviewer`）验 **实现**（QA 后，含 LLM）。可信度见 [quality-gates.md §1](./factory/quality-gates.md#1-在流水线中的位置)。
...
与 **`deploy_hitl`**（Reviewer 通过后敏感部署）职责分离；二者共用 `HitlDecision` Schema，靠 `stage` 区分。触顶原因写入 `HitlDecision.reason`（如 `loop_limit:impl_retry`）。字段见 [hitl.md](./factory/artifacts/schemas/hitl.md)、[quality-gates.md §5](./factory/quality-gates.md#5-hitl-节点spec_hitl--design_hitl--deploy_hitl--escalation_hitl)。
...
`docs/factory/runs/<task_id>/` 存档 JSON、MD、Mermaid（与 **设计 Spec** `docs/superpowers/specs/` 分离，见 §6.1）。多轮设计可选：`design.r2.json` 或子目录 `rounds/2/design.json`（实现择一，写入 `run_meta.artifact_layout`）。

#### 路由伪代码（程序真源）

实现：`multi_agent_code_factory/graph_routing.py`。validate 路由细节与 rule_id 见 [quality-gates.md](./factory/quality-gates.md)。
...
每次 run 写入 **`run_meta.json`**（Schema [run-meta.md](./factory/artifacts/schemas/run-meta.md)）：`loop_limits`、`profile` 快照、`budget` 用量、计数器、`checkpoint_id`、`deploy_status`。
...
| `reflection` | 若有（[dev-manifest.md](./factory/artifacts/schemas/dev-manifest.md)） |
...
- Checkpointer 默认：`docs/factory/runs/<task_id>/checkpoint.db`
...
**程序真源：** Run 目录下的 `.json`；人读 MD/Mermaid 为渲染摘要（[format-to-human/](./factory/artifacts/format-to-human/README.md)）。

| Schema | 生产者（role_id） | Run 机器文件 | Run 人读 | 字段规格 | 人读模板 |
|--------|-------------------|--------------|----------|----------|----------|
| `SpecArtifact` | PM（`pm`） | `spec.json` | `spec.md` | [schemas/spec.md](./factory/artifacts/schemas/spec.md) | [format-to-human/spec.md](./factory/artifacts/format-to-human/spec.md) |
| `DesignArtifact` | Architect（`architect`） | `design.json` | `design.md`、`flow.mmd` | [schemas/design.md](./factory/artifacts/schemas/design.md) | [format-to-human/design.md](./factory/artifacts/format-to-human/design.md)、[flow.md](./factory/artifacts/format-to-human/flow.md) |
| `DevManifest` | Developer（`developer`） | `dev_manifest.json` | — | [schemas/dev-manifest.md](./factory/artifacts/schemas/dev-manifest.md) | — |
| `TestReport` | QA（`qa`，Tool） | `test_report.json` | — | [schemas/test-report.md](./factory/artifacts/schemas/test-report.md) | — |
| `ReviewReport` | Reviewer（`reviewer`） | `review.json` | `review.md` | [schemas/review.md](./factory/artifacts/schemas/review.md) | [format-to-human/review.md](./factory/artifacts/format-to-human/review.md) |
| `ValidationReport` | validate 节点 | `*_validation.json` | — | [schemas/validation-report.md](./factory/artifacts/schemas/validation-report.md) | — |
| `HitlDecision` | HITL 节点 | `hitl.json` | — | [schemas/hitl.md](./factory/artifacts/schemas/hitl.md) | — |
| `RunMeta` | 引擎 | `run_meta.json` | — | [schemas/run-meta.md](./factory/artifacts/schemas/run-meta.md) | — |

**JSON 示例片段：** [factory/examples/](./factory/examples/README.md)（Todo / 多语言 context）。**完整嵌套类型与校验：** [artifacts/schemas/](./factory/artifacts/schemas/README.md)、[quality-gates.md](./factory/quality-gates.md)。路由与可信度见 **§4.3–§4.4**、[schemas/review.md](./factory/artifacts/schemas/review.md)、[quality-gates.md §1](./factory/quality-gates.md#1-在流水线中的位置)。

---

## 6. 代码布局

**原则：** 四层分离——**引擎**（`multi_agent_code_factory/`）、**设计 Spec**（`docs/superpowers/specs/`）、**run 审计**（`docs/factory/runs/`）、**生成代码**（Profile.`code_root`，**独立目录，不在本仓库**）。
...
├── docs/
│   ├── superpowers/specs/        # 【设计 Spec】人读架构文档（本文件所在处）
│   └── factory/
│       └── runs/                 # 【run 产物】每次 task 的结构化 Artifact + 人读摘要
│           └── <task_id>/
│               ├── run_meta.json # 含解析后的 code_root 绝对路径
│               ├── spec.json
...
│               ├── review.md
│               ├── hitl.json
│               └── checkpoint.db   # P1；LangGraph checkpointer
├── pyproject.toml
└── README.md
...
../arb-robot/                     # 示例：套利应用独立仓库 / 目录（profiles/arb）
├── core/
├── connectors/
├── strategies/
├── tests/
...
| 套利应用 | `../arb-robot/` | 独立业务仓库；`profiles/arb.yaml` |
...
| 路径 | 内容 | 谁写 |
|------|------|------|
| `docs/superpowers/specs/*.md` | 架构 **设计 Spec** | 人 / 设计阶段 |
| `docs/factory/runs/<task_id>/spec.json` | 单次 run 的 **SpecArtifact** | PM（`pm`） |
| `docs/factory/runs/<task_id>/spec.md` | 同上的人读摘要 | PM（`pm`） |
...
│   ├── read_file.py
│   ├── write_file.py
│   ├── write_artifact.py       # 写入 docs/factory/runs/<task_id>/
...
└── profiles/
    ├── default.yaml
    ├── go-cli.yaml
    ├── java-maven.yaml         # [profiles.md §3](./factory/profiles.md#3-profile-矩阵)
    ├── java-gradle.yaml
    ├── rust-cli.yaml
    ├── solidity-foundry.yaml
    ├── solidity-hardhat.yaml   # P2
    ├── arb.yaml                # P1；code_root 指向仓库外（如 ../arb-robot）
    ├── default/prompts/
    ├── java-maven/prompts/
    ├── java-gradle/prompts/
    ├── rust-cli/prompts/
    ├── solidity-foundry/prompts/
    └── arb/prompts/            # P1；套利领域 prompt
```
...
**State vs `run_meta`：** 图 State 仅持 **当前代** Artifact 与计数器；`run_meta.json` 持久化 `loop_limits` / `profile` 快照、`hitl_history[]`、`stale_artifacts[]`、`checkpoint_id`、`budget` 用量（字段见 [run-meta.md](./factory/artifacts/schemas/run-meta.md)）。
...
程序门、HITL 节点与 MVP 默认行为见 **§4.1.2**、[quality-gates.md §1–§2](./factory/quality-gates.md#1-在流水线中的位置)。

| MVP 默认（非生产 Profile） | 说明 |
|----------------------------|------|
| `spec_validate` / `design_validate` | **开启** |
| `spec_hitl` / `design_hitl` | 图节点存在；Profile **`require_hitl: false`**，不 interrupt |
| QA + Reviewer | 自动（受 `loop_limits` 约束） |
| `deploy_hitl` | 按 Profile.`hitl`；无敏感规则时可跳过 |
| 读写密钥、凭证 | **禁止**（全局） |
...
- [ ] `docs/factory/runs/<task_id>/` 下 **§5 所列 8 类** JSON 齐全（按需含 `*_validation.json`、`hitl.json`）；PM/Architect 含 `spec.md`、`design.md`、`flow.mmd`
...
- [ ] `spec` 含 `success_metrics`、`features`、`user_stories`、`requirement_pool`；`spec.md` 符合 [format-to-human/spec.md](./factory/artifacts/format-to-human/spec.md)；`design.md` 符合 [format-to-human/design.md](./factory/artifacts/format-to-human/design.md)
...
| 文档 | 内容 |
|------|------|
| [factory/references/glossary.md](./factory/references/glossary.md) | 术语表（原附录 A） |
| [factory/references/metagpt.md](./factory/references/metagpt.md) | MetaGPT 借鉴、B.3 backlog（原附录 B） |
| [factory/references/open-source-survey.md](./factory/references/open-source-survey.md) | 其它开源参考、C.5 backlog（原附录 C） |
| [factory/examples/](./factory/examples/README.md) | JSON / Mermaid 示例片段 |
| [factory/artifacts/](./factory/artifacts/README.md) | 产物 Schema 与人读模板 |
