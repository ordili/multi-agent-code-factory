...
引擎在 **Developer** 节点组装 system prompt 时，Profile 应附带 ``profiles/<language>/prompts/{language}-style-snippet.txt``（或等价 ``style-snippet.txt``），使模型生成源码时遵守本节。PM / Architect 等产出 JSON 或设计文档的角色 **不注入** 该摘要。其它语言（Go/Java/Rust/Solidity）使用同命名约定与 ``agents/llm/prompt/style_snippet.py`` 解析逻辑。
