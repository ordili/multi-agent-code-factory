...
审查 / 修订本目录 JSON 契约时 **仅以本表及正文字段定义为准**。`artifact-templates/`、`quality-gates/`、`examples/` 为**下游**，由彼引用本文，不在此表列出（见 [README.md §单向依赖](../README.md#1-单向依赖)）。
...
本目录 **只定义 Run `*.json` 的字段与类型**。人读格式（`spec.md`、`design.md` 等）、校验规则（`SPEC-*` / `DES-*`）、示例片段分别在 **`artifact-templates/`**、**`quality-gates/`**、**`examples/`** ——由下游引用本文，**本文不链接、不展开下游规格**（见 [README.md §单向依赖](../README.md#1-单向依赖)）。
