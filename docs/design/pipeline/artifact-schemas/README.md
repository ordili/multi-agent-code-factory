...
本目录 **只定义 Run `*.json` 的字段与类型**。校验规则（`SPEC-*` / `DES-*`）、示例片段分别在 **`quality-gates/`**、**`examples/`** ——由下游引用本文。人读格式（`spec.md`、`design.md` 等）为 **同名上游** `artifact-templates/*-spec.md`（JSON ↔ 人读 § 映射，列入各 spec 依赖表）；章节写法 **不在本文展开**（见 [README.md §单向依赖](../README.md#1-单向依赖)）。
...
| **只引上游** | 如 [design-spec.md](./design-spec.md) 引用 [prd-spec.md](./prd-spec.md)；同名 [artifact-templates/*-spec.md](../artifact-templates/) 列入依赖表（**人读模板**）；不引 `quality-gates/` |
