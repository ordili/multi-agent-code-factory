# prd-spec.md — PrdArtifact（PM JSON 契约）
...
> **实现：** `multi_agent_code_factory/schemas/prd.py`  
> **Run 路径：** `docs/runs/<task_id>/prd.json`（需求文档 JSON；原 `prd.json`，见 [PRD 产物命名](../../../superpowers/specs/2026-07-10-prd-artifact-rename-design.md)）
...
| `semantic_constraints` | SemanticConstraint[] | 可选；狭触发时非空（见 [semantic_constraints](#semantic_constraints)） |
...
术语表；与 prd-spec §2 对应。`schemas/prd.py` 中 `GlossaryEntry` 建模；经 `context.glossary[]` 传递，**PRD-118** 校验条目非空。
...
LLM 可输出 Connextra 整句；normalizer 可解析为 `{ as_a, want, so_that }`（见 `prd.py` `_USER_STORY_RE`）。
...
机器可判定的输入/行为边界；狭触发（自由输入、表达式解析、api|web|form 界面）时必须非空。校验见 [prd-validate §3.4](../quality-gates/prd-validate.md#34-语义校验prd-s01s06)。

### SemanticConstraint

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | string | 如 `SEM-IN-1` |
| `source_ref` | string | 引用 `US-*` / `FEAT-*` / `REQ-*` |
| `source_kind` | enum | `US` \| `FEAT` \| `REQ` |
| `kind` | enum | `input_shape` \| `command_shape` \| `output_shape` \| `state_transition` \| `invariant` |
| `summary` | string | 人读摘要 |
| `dimensions` | object | 键→规则字符串（`exactly:N`、`one_of:a,b`、`type:number` 等） |
| `excludes` | SemanticExclude[] | 明确排除项（可空） |
| `notes` | string? | 补充说明 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | string | 如 `SEM-EX-1` |
| `dimension` | string | 对应 `dimensions` 键 |
| `rule` | string | 规则字符串（如 `gte:3`） |
| `summary` | string | 人读摘要 |
    ...
    "excludes": [
      {
        ...
        "rule": "gte:3",
        "summary": "链式/混合运算（operand_count≥3）"
      }
    ]
  }
]
...
```json
{
  ...
  "consistency_profile": {
    ...
    "conflict_strategy": "not_applicable",
    "notes": "人读 prd.md 数据一致性为「无」；无跨请求/持久化状态"
  },
  "acceptance_criteria": [
    {
      "id": "AC-1",
...
| 字段 | 文档 | `schemas/prd.py` |
|------|------|-------------------|
| `context.glossary[]` | ✓ 约定 | `GlossaryEntry`；`context.glossary[]` 透传；**PRD-118** |
