...
1. Developer 读取 `spec`、`design`（及实现环 `retry_bundle`）。  
...
3. 引擎将 `source_files` 写入 `code_root`。  
...
| **RetryBundle** | `impl_retry_count > 0` 时注入 Developer prompt；含 `spec`、`design`、`test_report`、`dev_manifest`；`dev_manifest.reflection` 若存在则展开为 bundle 内 `reflection`；失败代码片段来自 `test_report.failures[].file` |
