...
| **规则**    | [prd-validate.md](./prd-validate.md)                                                | spec 校验通过后的 HITL 路由前提           |
...
> **规则清单：** [prd-validate.md](./prd-validate.md) · [design-validate.md](./design-validate.md)（HITL 无独立 `rule_id`，仅路由与 Profile 配置）

---

## 5. HITL 节点（prd_hitl / design_hitl / deploy_hitl / escalation_hitl）
...
| **写入** | `hitl.json`（`stage=escalation`）；`reason` 示例：`loop_limit:impl_retry`、`loop_limit:prd_revision` |
...
| | prd_validate / design_validate | Reviewer（LLM） |
|--|--------------------------------|-----------------|
| **时机** | Developer 前 | QA 后 |
| **对象** | 文档规则 | 代码 + 测试 + AC |
| **确定性** | 是 | 否 |

规则清单分别见 [prd-validate.md](./prd-validate.md)、[design-validate.md](./design-validate.md)。
