...
> **人读模板（定稿）：** [artifact-templates/design-spec.md](../artifact-templates/design-spec.md) · [flow-spec.md](../artifact-templates/flow-spec.md)

**rule_id 合计：** **58** 条（§4.1 JSON **38** · §4.2 `design.md` / `*.mmd` **19** · §4.4 HITL **1**）；**DES-224** 为 §5 新增；**4** 条 MD 规则 **已废弃**（见下表）。
...
> **实现同步：** 本节已按定稿 **§1–§6 + 附录 A–D** 更新；`design_md_rules.py` / `mermaid.py` / **DES-017** 代码 **待下一迭代对齐**（当前仍可能按旧 §1–§10 告警）。
...
| `DES-014` | error | 条件 | `requires_transaction_constraints(design, spec)` | `transaction_constraints` | 非空；无持久化 CLI 可为 `[]` |
| `DES-015` | error | 是 | — | `error_catalog` | 非空；≥1 条可映射 §4.7 异常分支（若写了 §4.7） |
...
Run `design.md` 须 **中文固定章节**（**§1–§6 + 附录 A–D**）；**不含** Rollout、方案对比专章、待澄清专章、监控专章。

### 硬必填章节（warn）

| rule_id | 严重度 | 必检 | 检查对象 | 判定 |
|---------|--------|------|----------|------|
| `DES-201` | warn | 是 | `design.md` | 须含 `## 1. 背景与上下文`、`## 2. 设计目标`、`## 3. 非目标`、`## 4. 方案设计`、`## 6. 测试用例列表` |
| `DES-202` | warn | 是 | `design.md` | 须含 `## 附录 A.`～`## 附录 D.`（需求追溯 / 文件变更 / 任务分解 / 与现有代码对照） |
| `DES-207` | warn | 是 | `design.md` | 须含 `## 附录 D. 与现有代码对照` |
...
| `DES-219` | warn | 是 | `test_cases` 非空 | `## 6` 表宜含「类型」列（happy / negative / boundary） |
| `DES-209` | warn | 是 | — | 须含 `## 3. 非目标`（可与 DES-201 合并告警） |
