...
| **上游模板** | [dev-principles-spec.md](./dev-principles-spec.md)                  | 代码审查工程基线            |
...
> **节点交接：** Run `review.json` 字段见下游 [artifact-schemas/review-spec.md](../artifact-schemas/review-spec.md)；本文件定义 Run `review.md` 章节与写法。  
> **Run 路径：** `docs/runs/<task_id>/review.md`  
> **路由语义：** [multi-agent-pipeline-design.md §4.3](../multi-agent-pipeline-design.md)（`route_after_review`）  
> **程序校验：** 无独立 `review_validate` 门禁；结构校验与路由守卫见下游 schemas / 总设计
...
| `approved=false` + `deploy`                         | **矛盾输出** — 路由语义见 [multi-agent-pipeline-design.md §4.3](../multi-agent-pipeline-design.md)（须回 Developer） |
...
`routing` **vs** `next_stage`**：** 报告级 `next_stage` **决定图路由**（见 [multi-agent-pipeline-design.md §4.3](../multi-agent-pipeline-design.md)）；单条 `Finding.routing` 供 Reviewer 自检「这条该谁修」，与 `next_stage` 宜一致。
...
Reviewer prompt 通常注入下列 Run 产物，对照**上游**人读模板与工程基线：
...
| `test_report`            | `passed`、失败用例、`tests_missing`（对照 [design-spec.md §6](./design-spec.md#6-测试用例) 路径约定） |
...
> **待同步（实现层）：** `reviewer` 节点 · `review_md` 渲染器 · `route_after_review`（以仓库实现为准）。  
> **以** `review.json` **为准**；MD 与 JSON 不一致时，路由只认 JSON。