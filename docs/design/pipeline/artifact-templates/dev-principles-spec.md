...
| 受众 | 语言 | 示例 |
|------|------|------|
| **人读** | 简体中文 | README 章节、CLI `--help`、面向用户的错误提示、公开 API 文档注释 |
| **机器读** | English | 源码标识符、JSON/配置键与枚举值、依赖清单元数据名、环境变量名、序列化 API 载荷 |

与流水线产物（`prd.json` / `design.json` 等）的 `artifact-language-snippet` 规则一致：叙述用中文，id / code / 路径 / API 名保持英文。
