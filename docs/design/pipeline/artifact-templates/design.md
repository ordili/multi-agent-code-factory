...
Todo 为 Profile=`default` **教学 fixture**；与 [`spec.md`](./spec.md)、[`schemas/design.md`](../artifact-schemas/design.md) 示例对照。下文为 **章节齐全的整篇样例**（§1–§11 + 附录 A–E）。

---

## 样例文档 — default Profile（CLI Todo）

> 对应 spec 示例「CLI Todo App」；Run 路径：`docs/runs/<task_id>/design.md`
...
- **时序图：** [flow-todo.mmd](../examples/snippets/flow-todo.mmd) — `add`（sequenceDiagram）
- **流程图：** [flow-todo.mmd](../examples/snippets/flow-todo.mmd) — 持久化与 ERR-STORE-* 分支（flowchart）
...
**长运行 / 多服务类任务：** 章节结构 **相同**；§4.3 改为 RPC/Redis/DB，§4.5 为真实表与索引，§9–§10 非 N/A。
