...
完整 Toolchain、Parser 矩阵、各语言 Profile YAML 与 CLI 示例见 **[profiles.md](./profiles.md)**。配置真源：[`multi_agent_code_factory/profiles/*.yaml`](../../../multi_agent_code_factory/profiles/)。
...
产物：`spec_validation.json`、`design_validation.json`（[`ValidationReport`](./artifact-schemas/validation-report.md)）。  
...
与 **`deploy_hitl`**（Reviewer 通过后敏感部署）职责分离；二者共用 `HitlDecision` Schema，靠 `stage` 区分。触顶原因写入 `HitlDecision.reason`（如 `loop_limit:impl_retry`）。字段见 [hitl.md](./artifact-schemas/hitl.md)、[quality-gates.md §5](./quality-gates.md#5-hitl-节点spec_hitl--design_hitl--deploy_hitl--escalation_hitl)。
...
每次 run 写入 **`run_meta.json`**（Schema [run-meta.md](./artifact-schemas/run-meta.md)）：`loop_limits`、`profile` 快照、`budget` 用量、计数器、`checkpoint_id`、`deploy_status`。
...
| `reflection` | 若有（[dev-manifest.md](./artifact-schemas/dev-manifest.md)） |
...
**程序真源：** Run 目录下的 `.json`；人读 MD/Mermaid 为渲染摘要（[artifact-templates/](./artifact-templates/README.md)）。

| Schema | 生产者（role_id） | Run 机器文件 | Run 人读 | 字段规格 | 人读模板 |
|--------|-------------------|--------------|----------|----------|----------|
| `SpecArtifact` | PM（`pm`） | `spec.json` | `spec.md` | [artifact-schemas/spec.md](./artifact-schemas/spec.md) | [artifact-templates/spec.md](./artifact-templates/spec.md) |
| `DesignArtifact` | Architect（`architect`） | `design.json` | `design.md`、`flow.mmd` | [artifact-schemas/design.md](./artifact-schemas/design.md) | [artifact-templates/design.md](./artifact-templates/design.md)、[flow.md](./artifact-templates/flow.md) |
| `DevManifest` | Developer（`developer`） | `dev_manifest.json` | — | [artifact-schemas/dev-manifest.md](./artifact-schemas/dev-manifest.md) | — |
| `TestReport` | QA（`qa`，Tool） | `test_report.json` | — | [artifact-schemas/test-report.md](./artifact-schemas/test-report.md) | — |
| `ReviewReport` | Reviewer（`reviewer`） | `review.json` | `review.md` | [artifact-schemas/review.md](./artifact-schemas/review.md) | [artifact-templates/review.md](./artifact-templates/review.md) |
| `ValidationReport` | validate 节点 | `*_validation.json` | — | [artifact-schemas/validation-report.md](./artifact-schemas/validation-report.md) | — |
| `HitlDecision` | HITL 节点 | `hitl.json` | — | [artifact-schemas/hitl.md](./artifact-schemas/hitl.md) | — |
| `RunMeta` | 引擎 | `run_meta.json` | — | [artifact-schemas/run-meta.md](./artifact-schemas/run-meta.md) | — |

**JSON 示例片段：** [examples/](./examples/README.md)（Todo / 多语言 context）。**完整嵌套类型与校验：** [artifact-schemas/](./artifact-schemas/README.md)、[quality-gates.md](./quality-gates.md)。路由与可信度见 **§4.3–§4.4**、[artifact-schemas/review.md](./artifact-schemas/review.md)、[quality-gates.md §1](./quality-gates.md#1-在流水线中的位置)。
...
**State vs `run_meta`：** 图 State 仅持 **当前代** Artifact 与计数器；`run_meta.json` 持久化 `loop_limits` / `profile` 快照、`hitl_history[]`、`stale_artifacts[]`、`checkpoint_id`、`budget` 用量（字段见 [run-meta.md](./artifact-schemas/run-meta.md)）。
...
- [ ] `spec` 含 `success_metrics`、`features`、`user_stories`、`requirement_pool`；`spec.md` 符合 [artifact-templates/spec.md](./artifact-templates/spec.md)；`design.md` 符合 [artifact-templates/design.md](./artifact-templates/design.md)
