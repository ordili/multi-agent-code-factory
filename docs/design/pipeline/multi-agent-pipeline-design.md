...
| **spec_validate / design_validate** MVP 规则集（[quality-gates/](./quality-gates/README.md)） | 全量 DES/SPEC 规则 + Mermaid 格式校验（P1；MVP 仅 Pydantic + 核心 error 规则） |
...
PM、Architect 产出在 **进入下游 Agent 之前** 须过 **spec_validate / design_validate**（程序规则）；Profile.`validation` 可配置 **spec_hitl / design_hitl**（人工，非 LLM Reviewer）。命名见 [quality-gates/README.md §0](./quality-gates/README.md#0-命名约定)。

| 节点 | 类型 | 失败 / 审批 | 细目 |
|------|------|-------------|------|
| **spec_validate** | 程序 | 回 **PM** | [spec-validate.md](./quality-gates/spec-validate.md) |
...
│   ├── __init__.py
│   ├── spec_rules.py
│   ├── spec_md_rules.py        # spec.md 格式（P1；quality-gates/spec-validate §3.3）
│   ├── design_rules.py
│   └── mermaid.py
