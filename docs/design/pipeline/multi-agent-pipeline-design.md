...
| [quality-gates/](./quality-gates/README.md) | PM / Architect **规则校验** + 可选 **人工 HITL** |
...
| **命名** | 图节点：`{产物}_validate` / `{产物}_hitl`；配置：`validation.*`；详见 [quality-gates/README.md §0](./quality-gates/README.md#0-命名约定) |
...
| **PM/Architect 质量** | **规则校验**（`prd_validate` / `design_validate`）+ 可选 **人工 HITL**（§4.1.2、[quality-gates/](./quality-gates/README.md)） |
...
**主线（文字摘要）：** PM → prd_validate → [prd_hitl?] → Architect → design_validate → [design_hitl?] → Developer → QA → Reviewer → [deploy_hitl?] → Deploy
...
规则校验见 [quality-gates/](./quality-gates/README.md)。
...
|----------|--------------------------------|
| **五 Agent** + validate/HITL/Deploy **图节点**（含 `deploy_hitl`；`prd_hitl` / `design_hitl` 图节点存在，Profile 默认 `require_hitl: false`） | `prd_hitl` / `design_hitl` **强制 interrupt**（生产级 Profile，P1 完善 UI） |
| **prd_validate / design_validate** MVP 规则集（[quality-gates/](./quality-gates/README.md)） | 全量 DES/SPEC 规则 + Mermaid 格式校验（P1；MVP 仅 Pydantic + 核心 error 规则） |
...
| Profile 矩阵：`java-maven`、`java-gradle`、`rust-cli`、`solidity-foundry`（[profiles.md §3](./profiles.md#3-profile-矩阵-v1)） | 多语言 **live e2e** 各一例（Parser 已实现，见 [developer-retry-context-spec.md](./developer-retry-context-spec.md)） |
...
**Markdown 仍要写：** 与 MetaGPT 一样，给人看的 PRD、设计说明不可省；**JSON 与 MD 同源生成**（先 Structured Output，再渲染 `prd.md`），或 MD 为 JSON 的摘要。章节模板见 [artifact-templates/](./artifact-templates/README.md)。
...
|---------|----------|--------------|-----------|------|
| `pm` | PM | `PrdArtifact` | `prd.md` | 需求、验收标准 |
...
与校验/HITL 节点（`prd_validate` 等，**非 Agent**）区分；五 Agent 的 **`role_id` = 图节点 id = `agents/` 模块名**：
...
**与 HITL 分工：** Reviewer（`reviewer`）= Test 后 **LLM** 审查；`prd_hitl` / `design_hitl` / `deploy_hitl` = **人工** interrupt 节点。
...
| 回路 | 触发 | 回到 | 计数器 | 配置项（默认） |
|------|------|------|--------|----------------|
| **需求环（validate）** | `prd_validate` 未通过 | PM | `prd_revision_count` | `max_prd_revisions`（**1**） |
...
| **需求环（Reviewer）** | Reviewer `next_stage=pm` | PM | `prd_revision_count` | 同上 |

路由比较：`count < loop_limits.*` 才允许回退；否则走 §4.4 `on_limit_exceeded`。升环后再入主线见 §4.3「再入规则」。  
Architect 重跑时读：`prd` + 上轮 `design` + **`design_validation`** + `review` 的 escalation；PM 重跑读 **`prd_validation`** 与 `review`（若来自 Reviewer 需求环）。
...
PM、Architect 产出在 **进入下游 Agent 之前** 须过 **prd_validate / design_validate**（程序规则）；Profile.`validation` 可配置 **prd_hitl / design_hitl**（人工，非 LLM Reviewer）。命名见 [quality-gates/README.md §0](./quality-gates/README.md#0-命名约定)。

| 节点 | 类型 | 失败 / 审批 | 细目 |
|------|------|-------------|------|
| **prd_validate** | 程序 | 回 **PM** | [prd-validate.md](./quality-gates/prd-validate.md) |
| **prd_hitl** | 人工 interrupt | 驳回 → PM | `validation.prd.require_hitl` |
| **design_validate** | 程序 | 回 **Architect** | [design-validate.md](./quality-gates/design-validate.md) |
| **design_hitl** | 人工 interrupt | 驳回 → Architect | `validation.design.require_hitl` 或 `hitl_flags` |

产物：`prd_validation.json`、`design_validation.json`（[`ValidationReport`](./artifact-schemas/validation-report-spec.md)）。  
**与 Reviewer 分工：** validate 验 **文档**（确定性）；Reviewer（`reviewer`）验 **实现**（QA 后，含 LLM）。可信度见 [quality-gates/README.md §1](./quality-gates/README.md#1-在流水线中的位置)。

### 4.2 角色与 Tool

| role_id | 文档称呼 | 机器产物 | 人读 / 图 | 代码产出 | 读取 | Tool |
|---------|----------|----------|-----------|----------|------|------|
| `pm` | PM | `prd.json` | `prd.md` | — | `user_request`、`profile` | `write_artifact` → **prd_validate** |
| `architect` | Architect | `design.json` | `design.md`、`flow.mmd` | — | `prd`、`profile`；重跑 + `prd_validation` / `design_validation` | `write_artifact` → **design_validate** |
| `developer` | Developer | `dev_manifest.json` | — | `{code_root}/**` | `prd`、`design`、`profile` | Profile.`tools` |
| `qa` | QA | `test_report.json` | — | — | `design`、`dev_manifest`、`profile` | `run_tests` |
| `reviewer` | Reviewer | `review.json` | `review.md` | — | `prd`、`design`、`test_report`、`dev_manifest`、diff | 读 `code_root` |
...
|------|------|-----------------------------------|--------------|
| `prd_validate` 未通过 | PM | **prd_validate** | Architect → design_validate → Developer → QA → Reviewer |
| Reviewer `next_stage=pm` | PM | **prd_validate** | 同上；`design.revision++`，`test_report` / `review` **作废** |
...
**`code_root` 与升环：** Reviewer 升环至 `architect` / `pm` 时，`{code_root}` 下已写源码 **不自动删除**；`dev_manifest.json` 标记为 stale 并写入 `run_meta.stale_artifacts[]`。下一轮 Developer 须对照新版 `design` / `prd` 修订或废弃冲突文件（prompt 注入 stale 警告）。仅实现环（测试失败 / Reviewer `next_stage=developer` / `deploy_hitl` 驳回续跑）时 **不** 标记 code stale。
...
- **prd_validate 失败** → PM，`prd_revision_count += 1`；注入 `prd_validation.violations`。
...
- Reviewer `next_stage=pm` → PM，`prd_revision_count += 1`。
- Developer `needs_architect=true` 仅作 **Reviewer 输入提示**，路由以 Reviewer `next_stage` 为准。

#### prd_hitl / design_hitl / deploy_hitl

- **prd_hitl：** `validation.prd.require_hitl=true` → interrupt；人读 `prd.md` + `prd_validation.json`；驳回 → PM。
...
| **继续** | `approved=true` → 人工指定重置：`impl_retry_count` / `design_revision_count` / `prd_revision_count` 归零或下调；从 [§4.3 再入规则](#再入规则升环--validate-失败后) 中 **与触顶回路对应** 的再入点续跑（如实现环触顶 → Developer，设计 validate 触顶 → Architect，需求 validate 触顶 → PM） |

与 **`deploy_hitl`**（Reviewer 通过后敏感部署）职责分离；二者共用 `HitlDecision` Schema，靠 `stage` 区分。触顶原因写入 `HitlDecision.reason`（如 `loop_limit:impl_retry`）。字段见 [hitl-spec.md](./artifact-schemas/hitl-spec.md)、[hitl.md §5](./quality-gates/hitl.md)。
...
实现：`multi_agent_code_factory/graph_routing.py`。validate 路由细节与 rule_id 见 [quality-gates/](./quality-gates/README.md)。

```python
def route_after_prd_validate(state, profile, limits: LoopLimits) -> str:
    v = state.prd_validation
    if not v.passed and profile.validation.prd.block_on == "error":
...
| `max_prd_revisions` | 1 | 需求环上限（validate + Reviewer 升环 **共用**） |
...
|---------|----------|----------------|
| `pm` | PM | `user_request`、`profile`；需求环重跑 + `prd_validation`、（Reviewer 升环时）`review` |
| `architect` | Architect | `prd`、`profile`；设计环 + `review`、`test_report`、`design_validation` |
| `developer` | Developer | `prd`、`design`、`profile`；实现环 + **RetryBundle** |
| `qa` | QA | `design`、`dev_manifest`、`profile` |
| `reviewer` | Reviewer | `prd`、`design`、`test_report`、`dev_manifest`、diff |
...
当 `impl_retry_count > 0`，Developer **必须** 注入（详见 [developer-retry-context-spec.md](./developer-retry-context-spec.md)）：

| 顶层字段 | 首轮 | 重试 |
|----------|------|------|
| `prd` / `design` | 有 | 有（**不在** `retry_bundle` 内） |
| `impl_mode` | `initial` | `retry_patch` |

| `retry_bundle` 字段 | 来源 |
|---------------------|------|
| `retry_cause` | `qa_failure` / `review_rejection` / `both` |
| `test_report` | 最近 QA 报告（含 `failures[]`） |
| `review_feedback` | Reviewer 打回时（`findings[]`） |
| `dev_manifest` | 上轮 `changed_files` |
| `reflection` | 若有（[dev-manifest-spec.md](./artifact-schemas/dev-manifest-spec.md)） |
| `failure_contexts` | call-path 栈帧 + 函数体 snippet（全局预算） |
...
- 水合 JSON 产物 → 推断再入点 → **先跑门禁**（`prd_validate` / `design_validate` / `qa`）→ 按需 LLM
...
| 场景 | 做法 |
|------|------|
| 新功能 | 新 `task_id`；`prd.parent_task_id` 指向父 run |
| 小改需求 | `prd.revision++`，PM 合并 `scope_in` |
...
实现：`multi_agent_code_factory/schemas/`（Pydantic v2）。所有 Artifact 含 `version: "1"`。Schema **领域无关**；领域字段在 `PrdArtifact.context`（Profile 可选 `context_schema`）。
...
|--------|-------------------|--------------|----------|----------|----------|
| `PrdArtifact` | PM（`pm`） | `prd.json` | `prd.md` | [artifact-schemas/prd-spec.md](./artifact-schemas/prd-spec.md) | [artifact-templates/prd-spec.md](./artifact-templates/prd-spec.md) |
...
**JSON 示例片段：** [examples/](./examples/README.md)（Todo / 多语言 context）。**完整嵌套类型与校验：** [artifact-schemas/](./artifact-schemas/README.md)、[quality-gates/](./quality-gates/README.md)。路由与可信度见 **§4.3–§4.4**、[artifact-schemas/review-spec.md](./artifact-schemas/review-spec.md)、[quality-gates/README.md §1](./quality-gates/README.md#1-在流水线中的位置)。
...
| `docs/runs/<task_id>/prd.json` | 单次 run 的 **PrdArtifact** | PM（`pm`） |
| `docs/runs/<task_id>/prd.md` | 同上的人读摘要 | PM（`pm`） |
...
│   ├── __init__.py
│   ├── prd_rules.py
│   ├── prd_md_rules.py        # prd.md 格式（P1；quality-gates/prd-validate §3.3）
│   ├── design_rules.py
│   └── mermaid.py
...
prd_validation: ValidationReport | None
...
prd_revision_count: int
```

`LoopLimits`：`max_impl_retries`, `max_design_revisions`, `max_prd_revisions`, `on_limit_exceeded`。`max_hitl_rounds` 见 §4.4（P1）。
...
程序门、HITL 节点、`loop_limits` 与 MVP 默认行为见 **§4.1.2**、**§4.4**、[quality-gates/README.md §0–§2](./quality-gates/README.md#0-命名约定)。V1 默认 Profile（如 `default`）通常 `validation.*.require_hitl: false`；生产级 `require_hitl: true` 为 P1。
...
- [ ] **`prd_validate` / `design_validate`** MVP 规则跑通；失败回 PM/Architect；升环再入符合 §4.3 **[MVP]**
- [ ] `prd_hitl` / `design_hitl` interrupt 可暂停与恢复（P1）
- [ ] `docs/runs/<task_id>/` 下 **§5 所列 8 类** JSON 齐全（按需含 `*_validation.json`、`hitl.json`）；PM/Architect 含 `prd.md`、`design.md`、`flow.mmd`
...
- [ ] `prd.context.language` 与 Profile.`language` 一致（或显式 warn） **[MVP]**
- [x] `go_json` / `cargo_json` / `forge_json` Parser 注册 + 单元测试（live e2e 各一例仍 P1）
...
- [x] §4.5 `watch` 注入符合角色表；Developer 重试含瘦身 RetryBundle + `failure_contexts`（[developer-retry-context-spec.md](./developer-retry-context-spec.md)）
- [ ] `resume --task-id` 从 checkpoint 续跑（P1）
- [ ] `prd` 含 `features`、`user_stories`、`requirement_pool`（`success_metrics` 可选）；`prd.md` 符合 [artifact-templates/prd-spec.md](./artifact-templates/prd-spec.md)；`design.md` 符合 [artifact-templates/design-spec.md](./artifact-templates/design-spec.md)
